...
const typeOfMember = [
  {
    type: "checkbox",
    message: "Type of team member would you like to add?",
    name: "member1", 
    choices: ["engineer","intern"]  
  }
]
...
inquirer
  ...
  .then(function(user) {
    ...
    //adding team members
    addTeamMember(proceed,user.member[0],team,templateMainFile);
...
async function addTeamMember(proceed,chosenMember,team, templateMainFile) {
  try {
    do{
      switch(chosenMember){
        case "engineer":
        ...
        if(nextMove.choice[0]==="false"){
            ...
            fs.writeFileSync("./output/output.html",temporaryMainFile);
